...
def test_create_embedding_matrix_with_index():
    ...
    merged_embeddings = pd.DataFrame(embeddings_data, index=['ENSG1', 'ENSG2', 'ENSG3', 'ENSG4'])
    major_ensembl_ids = pd.Series(['ENSG1', 'ENSG2', 'ENSG4'])
...
def test_create_embedding_matrix_with_gene_id_index():
    ...
    assert torch.allclose(embedding_matrix_torch, torch.tensor(expected_matrix, dtype=torch.float32))
