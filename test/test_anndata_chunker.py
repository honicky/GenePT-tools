    ...
    var = pd.DataFrame(
        {
            ...
            "feature_type": pd.Categorical(
                ["protein_coding" if i % 2 else "lncRNA" for i in range(n_vars)]
            ),
        }
    )
...
def test_helper_functions(temp_h5ad_file):
    ...
    with h5py.File(temp_h5ad_file, "r") as f:
        ...
        obs_df = _load_obs_metadata(
            f, start_row=0, n_rows=10, obs_columns=["cell_type", "condition"]
        )
    ...
    with AnnDataChunker(temp_h5ad_file, ["cell_type", "condition"]) as chunker:
        ...
        for col in ["cell_type", "condition"]:
            pd.testing.assert_series_equal(
                concatenated.obs[col], full_data.obs[col], check_names=False
            )

        # Test that sparse matrix data matches
        assert np.allclose(concatenated.X.toarray(), full_data.X.toarray())
