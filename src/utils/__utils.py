...
def extract_gz(
  gz_path: Union[Path, str], extract_dir: Union[Path, str] = Path("data")) -> Path:
  """
...
class AnnDataChunker:
  ...
  def _load_index_column(self, f):
    ...
    self._index_column = (
      f["obs"].attrs["_index"] if "_index" in f["obs"].attrs else
      f["obs"].attrs["index"] if "index" in f["obs"].attrs else "_index")
  ...
  def load_subset(self, start_row, n_rows, valid_indices=None):
    ...
    data, indices, indptr = _load_csr_matrix_components(self._file, start_row, n_rows,
                                                        valid_indices)
    ...
    var_df = (
      self._var_df.iloc[valid_indices] if valid_indices is not None else self._var_df)
...
def load_subset_anndata(file_path, start_row=0, n_rows=None, obs_columns=None):
  ...
  with h5py.File(file_path, "r") as f:
    ...
    X_subset = sparse.csr_matrix((data, indices, indptr), shape=(n_rows, len(var_df)))
...
def _load_var_metadata(f):
  ...
  for col in var_df.columns:
    if var_df[col].dtype == object:
      var_df[col] = var_df[col].apply(lambda x: x.decode("utf-8")
