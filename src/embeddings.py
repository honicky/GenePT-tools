...
def load_data_set(data_set_directory_path: Path, label_column: str) -> pd.DataFrame:

  first_path = next(iter(glob.glob(str(data_set_directory_path / '*.parquet'))))
  first_columns = pd.read_parquet(first_path).columns
  embedding_columns = [
    col
  ...
  test_set_dataframes = (
    pd.read_parquet(f)[embedding_columns + [label_column]]
    for f in glob.glob(str(data_set_directory_path / '*.parquet'))
  )
  return pd.concat(test_set_dataframes)