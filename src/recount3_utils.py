...
def create_rse_manual(project, project_home, organism, annotation, type):
  ...
  if "sample_attributes" in metadata_df.columns:
    sample_attributes = parse_attributes(metadata_df, "external_id",
                                         "sample_attributes")

    # First merge metadata with sample attributes
    metadata_with_attrs = metadata_df.merge(
      sample_attributes, left_on="external_id", right_on="external_id", how="left")
    ...
    result = counts_long.merge(metadata_with_attrs, on="external_id", how="left")
