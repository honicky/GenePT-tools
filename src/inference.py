...
def create_gene_embedding_matrix(gene_names: list[str], gene_embedding: pd.DataFrame) -> np.ndarray:
    """
...
def gene_pt_w_embedding(gene_expression_counts: pd.DataFrame, experiment_ids: list[str], gene_embedding: pd.DataFrame) -> np.ndarray:
    """
    ...
    lookup_embed = create_gene_embedding_matrix(
        gene_expression_counts.gene_name, 
        gene_embedding
    )
    ...
    unnormalized_gene_pt_w_embedding = np.dot(gene_expression_counts[experiment_ids].T, lookup_embed)
    # print(f"unnormalized_gene_pt_w_embedding.shape: {unnormalized_gene_pt_w_embedding.shape}")
    normalized_gene_pt_w_embedding = unnormalized_gene_pt_w_embedding/np.linalg.norm(unnormalized_gene_pt_w_embedding, axis=1, keepdims=True)
    return pd.DataFrame(normalized_gene_pt_w_embedding, index=experiment_ids)

def gene_pt_w_embedding_normalized(gene_expression_counts: pd.DataFrame, experiment_ids: list[str], gene_embedding: pd.DataFrame) -> np.ndarray:
    return gene_pt_w_embedding(gene_expression_counts, experiment_ids, gene_embedding)/np.linalg.norm(gene_pt_w_embedding(gene_expression_counts, experiment_ids, gene_embedding), axis=1, keepdims=True)